...
profile parses with 446 operations, but lacks the current containment, inventory,
and workflow evidence needed for reference qualification.

The coordinator must pass the same selection to capture and every workflow
suite, verify the actual resulting inventory, and refuse an unexpected module
state or substituted profile. IA documents fully qualified identifiers and the
