...
- Use `--fields` with `--json` when you need a subset object (for example: `igw doctor --json --fields ok,checks.0.name`).
...
- `igw call` and `igw doctor` support `--fields` for subset JSON object output from `--json` envelopes.
