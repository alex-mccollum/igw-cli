...
func TestTransferObservationCountsActualCLIRequests(t *testing.T) {
	...
	app := nextcli.App{In: strings.NewReader(""), Out: &out, Err: io.Discard, CacheDir: t.TempDir(), HTTP: client, Getenv: func(string) string { return "" }, ReadConfig: func() (config.File, error) { return config.File{GatewayURL: srv.URL, Token: "synthetic-test-token"}, nil }}
