...
func testLiveOperations(t *testing.T, workflows bool) {
	...
	if path := os.Getenv("IGW_OPERATIONS_EVIDENCE"); path != "" {
		...
		}{2, "operations-contract", image, s.ImageID, s.Platform, s.ModuleInventory, s.Modules, s.GatewayVersion, hex.EncodeToString(h.Sum(nil)), started, time.Now().UTC(), synced.Meta.Catalog, checks, artifacts, true, true}
		if workflows {
