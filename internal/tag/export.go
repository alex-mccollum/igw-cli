...
func Export(runner Runner, input ExportRequest) result.Result {
	...
	if required := runner.Require(exportCapability()); !required.OK {
		return required
	}
	...
	return runner.Run(execute.Request{Operation: exportOperation, Query: query, Out: input.Out, Overwrite: input.Overwrite})
}
